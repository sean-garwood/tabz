...
const { createChromeMock, loadScript, evalIn, windowOrder, tabById } = require("./chrome-mock.js");

function setup(tabs, groups) {
  const { chrome, state } = createChromeMock({ tabs, groups });
  const ctx = loadScript("background.js", { chrome });
  return {
    state,
    ctx,
    handle: ctx.handleMessage,
...
test("unpinned tab cannot move into the pinned region", async () => {
  ...
  assert.deepEqual(windowOrder(state, 1), [1, 2, 3, 4]);
});
...
test("pinned tab stays inside the pinned region", async () => {
  ...
  assert.deepEqual(windowOrder(state, 1), [2, 1, 3, 4]);
});
...
test("moveEdge start lands just after the pinned region", async () => {
  ...
  assert.deepEqual(windowOrder(state, 1), [1, 2, 4, 3]);
});
...
test("createGroup auto-names from the hostname and picks a palette color", async () => {
  const { state, handle, sender, ctx } = setup([{ id: 1, url: "https://www.github.com/foo" }]);
  const res = await handle({ type: "createGroup" }, sender(1));
  assert.equal(res.ok, true);
  assert.equal(res.notice, "Grouped: github.com");
  assert.equal(tabById(state, 1).groupId, 100);
  assert.equal(state.groups[100].title, "github.com");
  assert.ok(evalIn(ctx, "GROUP_COLORS").includes(state.groups[100].color));
});

test("groupColorFor is deterministic", () => {
  const { ctx } = setup([]);
  assert.equal(ctx.groupColorFor("github.com"), ctx.groupColorFor("github.com"));
});

test("groupTitleFor falls back and strips www", () => {
  const { ctx } = setup([]);
  assert.equal(ctx.groupTitleFor("https://www.github.com/x"), "github.com");
  assert.equal(ctx.groupTitleFor("chrome://settings/"), "settings");
  assert.equal(ctx.groupTitleFor("not a url"), "tabs");
});
...
test("joinGroup prefers the left group on a distance tie", async () => {
  ...
  assert.equal(res.notice, "Joined: left");
  assert.equal(tabById(state, 2).groupId, 5);
});
...
test("joinGroup falls back to the nearest group on the right", async () => {
  ...
  const res = await handle({ type: "joinGroup" }, sender(2));
  assert.equal(res.notice, "Joined: right");
  assert.equal(tabById(state, 2).groupId, 7);
});
...
test("joinGroup reports when no group exists", async () => {
  ...
  assert.deepEqual(res, { ok: false, notice: "No group nearby" });
});
...
test("ungroup removes only the current tab", async () => {
  ...
  assert.equal(res.ok, true);
  assert.equal(tabById(state, 1).groupId, -1);
  assert.equal(tabById(state, 2).groupId, 5);
});
...
test("ungroup reports when the tab is not grouped", async () => {
  ...
  const res = await handle({ type: "ungroup" }, sender(1));
  assert.deepEqual(res, { ok: false, notice: "Not in a group" });
});
...
test("dissolveGroup ungroups every member and nothing else", async () => {
  ...
  assert.equal(res.notice, "Ungrouped 2 tabs");
  assert.equal(tabById(state, 1).groupId, -1);
  assert.equal(tabById(state, 2).groupId, -1);
  assert.equal(tabById(state, 3).groupId, 9);
});
...
test("countMatches rejects an invalid regex", async () => {
  ...
  assert.deepEqual(res, { ok: false, notice: "Invalid regex" });
});
...
test("closeMatches removes exactly the matching tabs", async () => {
  ...
  assert.equal(res.notice, "Closed 2 tabs");
  assert.deepEqual(state.removed.sort(), [1, 2]);
  assert.deepEqual(windowOrder(state, 1), [3]);
  assert.deepEqual(windowOrder(state, 2), [4]);
});

test("closeMatches with no matches removes nothing", async () => {
  const { state, handle, sender } = setup(REGEX_FIXTURE);
  const res = await handle({ type: "closeMatches", pattern: "zzzzz" }, sender(1));
  assert.deepEqual(res, { ok: true, count: 0, notice: "No matches" });
...
test("keyboard command acts on the active tab", async () => {
  ...
  assert.deepEqual(windowOrder(state, 1), [2, 1, 3]);
});
