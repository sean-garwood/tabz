...
function payloadOf(res: TabzResponse): TabzConfigPayload {
    if (!res.ok || !res.config) throw new Error("Expected a config payload");
    return res.config;
}
...
test("getConfig falls back to defaults when the stored config is invalid", async () => {
