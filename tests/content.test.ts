...
function feedAll(parser: ReturnType<typeof createSequenceParser>, keys: string[]) {
...
test("sw moves left by one", () => {
  ...
  expect(results[1]).toEqual({ handled: true, command: { type: "move", delta: -1 } });
});
...
test("bare s0 moves to the start and s$ to the end", () => {
  const { parser } = makeParser();
  expect(feedAll(parser, ["s", "0"])[1].command).toEqual({ type: "moveEdge", edge: "start" });
  expect(feedAll(parser, ["s", "$"])[1].command).toEqual({ type: "moveEdge", edge: "end" });
});
...
test("group and prompt sequences", () => {
  const { parser } = makeParser();
  expect(feedAll(parser, ["s", "c"])[1].command).toEqual({ type: "createGroup" });
  expect(feedAll(parser, ["s", "a"])[1].command).toEqual({ type: "joinGroup" });
  expect(feedAll(parser, ["s", "q"])[1].command).toEqual({ type: "ungroup" });
  expect(feedAll(parser, ["s", "Q"])[1].command).toEqual({ type: "dissolveGroup" });
...
test("isEditableTarget recognizes inputs and contenteditable", () => {
  const evt = (el: unknown) => ({ composedPath: () => [el], target: el });
  expect(isEditableTarget(evt({ nodeType: 1, tagName: "INPUT" }))).toBe(true);
  expect(isEditableTarget(evt({ nodeType: 1, tagName: "TEXTAREA" }))).toBe(true);
  expect(isEditableTarget(evt({ nodeType: 1, tagName: "DIV", isContentEditable: true }))).toBe(
    true
  );
  expect(isEditableTarget(evt({ nodeType: 1, tagName: "DIV" }))).toBe(false);
  expect(isEditableTarget({ target: { nodeType: 1, tagName: "INPUT" } })).toBe(true);
